...
def create_inbound(db: Session, inbound_data: dict):
    db_inbound = models.Inbound(**inbound_data)
    db.add(db_inbound)
    db.commit()
...
def update_inbound(db: Session, inbound_id: int, inbound_data: dict):
    ...
    if db_inbound:
        for key, value in inbound_data.items():
            if hasattr(db_inbound, key): setattr(db_inbound, key, value)
        db.commit()
        db.refresh(db_inbound)
    return db_inbound
...
def get_subscription_by_token(db: Session, token: str):
    return db.query(models.Subscription).filter(models.Subscription.sub_token == token).first()
...
def update_subscription(db: Session, sub_id: int, sub_data: dict):
    ...
    if db_sub:
        for key, value in sub_data.items():
            if value is not None: setattr(db_sub, key, value)
        db.commit()
        db.refresh(db_sub)
    return db_sub

def reset_traffic_for_subscription(db: Session, subscription_id: int):
    db.query(models.Client).filter(models.Client.subscription_id == subscription_id).update({
        models.Client.up_traffic: 0,
        models.Client.down_traffic: 0
    })
    db.commit()
